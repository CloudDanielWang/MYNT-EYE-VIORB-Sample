...
int main(int argc, char **argv)
{
    ...
    {
        imagesub = nh.subscribe(config._imageTopic, /*200*/ 50, &ORBVIO::MsgSynchronizer::imageCallback, &msgsync);
    ...
    if(!ORB_SLAM2::ConfigParam::GetRealTimeFlag())
    {
        ...
        BOOST_FOREACH(rosbag::MessageInstance const m, view)
        {
            ...
            if(bdata)
            {
                ...
                ROS_INFO("image time: %.4f",imageMsg->header.stamp.toSec());
                for(unsigned int i=0;i<vimuMsg.size();i++)
                {
                    ...
                    cout << "imusize" << vimuMsg.size() << "time" << imuMsg->header.stamp.toSec() << endl;
                    ROS_INFO("imu time: %.4f",vimuMsg[i]->header.stamp.toSec());
                }
            ...
            if(bdata)
            {
                ...
                for(unsigned int i=0;i<vimuMsg.size();i++)
                {
                    ...
                    ORB_SLAM2::IMUData imudata(imuMsg->angular_velocity.x,imuMsg->angular_velocity.y,imuMsg->angular_velocity.z,
                                               ax,ay,az,imuMsg->header.stamp.toSec());
                    vimuData.push_back(imudata);
                    //ROS_INFO("imu time: %.5f",vimuMsg[i]->header.stamp.toSec());
                }
                ...
                // cout << "image_time_message" << imageMsg->header.stamp.toSec() << std::endl;
